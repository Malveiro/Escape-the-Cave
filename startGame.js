let startScreen = (function(input) {
    ...
    // current frame
    let wasButtonDown = false;

    // a helper function
    // used internally to draw the text in
    // in the center of the canvas (with respect
    // to the x coordinate)
    function centerText(myGameArea.context, text, y) {
        let measurement = myGameArea.context.measureText(text);
        let x = (myGameArea.context.canvas.width - measurement.width) / 2;
